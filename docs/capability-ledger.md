...
What a GPU management CLI can offer, and what xe-gmi 0.2.0 does on Battlemage/DG2 under the xe driver.

The complete surface a GPU management CLI can offer, classified for xe-gmi 0.1.2 on Battlemage/DG2
under the xe driver. Statuses: **mapped** (implemented), **degraded** (implemented with a documented
difference in meaning or scope), **N/A-interface** (the kernel exposes no interface; nothing xe-gmi
can do without kernel changes), **N/A-scope** (an interface exists but 0.1.1 deliberately leaves it
out; listed with the path so a later release can add it), **impossible-without-kernel-work** (the
only route is debugfs/ioctl/firmware and is excluded by this product's safety rules).
...
| Device recovery after a wedge | mapped (0.2.0) | `recover` (crash save, unbind, flr/bus reset or rebind, re-apply persistence) behind the preconditions in docs/hardware-safety.md |
...
| Capability | 0.1.1 | 0.1.2 | 0.2.0 | Interface / note |
|---|---|---|---|---|
| NUMA node, local CPUs, IOMMU group, PCI path | — | Full | Full | PCI sysfs |
| Multi-GPU affinity matrix | — | Full (PIX/PHB/NODE/SYS, own definitions) | Full | PCI ancestry + NUMA |
| P2P capability probing | — | Excluded | Excluded | would submit GPU work |
| AER error counters (endpoint, root port) | — | Full where exposed / N/A otherwise | same | `aer_dev_*`, `aer_rootport_*` |
| Crash dumps: list/show/save/release | — | Full | Full | devcoredump |
| Survivability mode/info | — | Read (root) | Read | xe sysfs 6.15+ |
| Wedged detection | — | Kernel log (root) | + live events | kmsg; uevent WEDGED |
| Power-limit averaging windows | Read | Read/Write, persisted | same | hwmon `*_interval` |
| fdinfo memory: all stats × all regions | Partial | Full (JSON) | Full | fdinfo |
| Process grouping (client/cgroup/user), `pmon` | — | Full | Full | fdinfo + procfs |
| cgroup dmem view | — | Full | Full | cgroup v2 6.14+ |
| cgroup dmem limits | — | — | Write (`cgroup set`) | `dmem.max` |
| SR-IOV state and profiles | — | Read | Read/Write (enable/disable/vf set/stop) | PCI + `sriov_admin` 7.0/7.1 |
| Connectors | — | Read | Read | DRM sysfs |
| Support bundle | — | Full (directory, redacted) | Full | — |
| Provenance per field | — | Full | Full | — |
| Kernel memory regions (authoritative used/total, small-BAR) | — | — | Full (render node) | DRM query 1 |
| Engine/GT/EU topology | — | — | Full | DRM queries 0/3/5 |
| GuC/HuC versions | — | — | Full (6.9+) | DRM query 7 |
| Device configuration (VA bits, alignment) | — | — | Full | DRM query 2 |
| RAS counters, clear | — | — | Full on 7.2+ Battlemage (root) | DRM RAS netlink |
| Event stream | — | — | Full (root) | uevent netlink |
| Device recovery (`recover`) | — | — | Full (flr/bus/rebind, preconditions) | PCI reset, driver bind |
| Firmware flashing, fan control, ECC mode, OA/EU-stall, `gpu_health`, Xe configfs | Excluded | Excluded | Excluded | reasons above and in docs/compat.md |
